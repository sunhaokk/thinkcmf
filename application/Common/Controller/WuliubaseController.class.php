...
class WuliubaseController extends AppframeController {

	public function __construct() {
	    hook('admin_begin');
		$admintpl_path=C("SP_WULIU_TMPL_PATH").C("SP_WULIU_DEFAULT_THEME")."/";
		C("TMPL_ACTION_SUCCESS",$admintpl_path.C("SP_WULIU_TMPL_ACTION_SUCCESS"));
		C("TMPL_ACTION_ERROR",$admintpl_path.C("SP_WULIU_TMPL_ACTION_ERROR"));
		parent::__construct();
		$time=time();
	...
	function _initialize(){
	    parent::_initialize();
	    define("TMPL_PATH", C("SP_WULIU_TMPL_PATH"));
	    //暂时取消后台多语言
	    $this->load_app_admin_menu_lang();	    
	    $session_admin_id=session('WULIU_ID');
    	if(!empty($session_admin_id)){
			...
			if(IS_AJAX){
				$this->error("您还没有登录！",U("wuliu/public/login"));
			}else{
				header("Location:".U("wuliu/public/login"));
				exit();
			}
	...
	public function initMenu() {
		$Menu = F("OswWuliuMenu");
		if (!$Menu) {
			$Menu=D("Common/OswWuliuMenu")->menu_cache();
		}
		return $Menu;
	}
	...
	public function parseTemplate($template='') {
		$tmpl_path=C("SP_WULIU_TMPL_PATH");
		define("SP_TMPL_PATH", $tmpl_path);
    	if($this->theme) { // 指定模板主题
    	    $theme = $this->theme;
    	}else{
    	    // 获取当前主题名称
    	    $theme      =    C('SP_WULIU_DEFAULT_THEME');
    	}
	...
	private function check_access($uid){
		...
		$no_need_check_rules=array("WuliuIndexindex","WuliuMainindex");
		if( !in_array($rule,$no_need_check_rules) ){
			return sp_auth_check($uid, null, 'or', 'OswWuliu');
		}else{
			return true;
		}
	}
    ...
    private function load_app_admin_menu_lang(){
	    ...
	    $langSet=C('WULIU_LANG_SWITCH_ON',null,false)?LANG_SET:$default_lang;
