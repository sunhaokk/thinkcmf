...
hello;
    ...
    private function _do_mobile_login(){
        ...
        if(!empty($result)){
            if(sp_compare_password($password, $result['user_pass'])){
                ...
                $this->get_user_co_info($result['id']);
    ...
    private function get_user_co_info($user_id)
    {

        $where = array();
        $where['user_id']=$user_id;//前面验证登录成功时获取到user_id
        $trade = M('OswTrade');
        $result = $trade->where($where)->find();
        if(!$result){
            $tradeuser = M('OswTradeUser');
            //取出trade ID
            $result = $tradeuser->where($where)->find();
            //根据TRADE_ID 取出 trade
            $where = array('trade_id' => $result['trade_id']);
            $result = $trade->where($where)->find();
        }
        if($result){
            session('trade',$result);
        }

        $where = array();
        $wuliu = M('OswWuliu');
        $where['user_id']=$user_id;//前面验证登录成功时获取到user_id
        $result = $wuliu->where($where)->find();
        if(!$result){
            $wuliuuser = M('OswWuliuUser');
            //取出trade ID
            $result = $wuliuuser->where($where)->find();
            //根据TRADE_ID 取出 wuliu
            $where = array('wuliu_id' => $result['trade_id']);
            $result = $wuliu->where($where)->find();
        }
        if($result){
            session('wuliu',$result);
        }
    }
}