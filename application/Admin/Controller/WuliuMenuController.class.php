...
class WuliuMenuController extends AdminbaseController {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->menu_model = D("Common/OswWuliuMenu");
        $this->auth_rule_model = D("Common/OswWuliuAuthRule");
    }
    ...
    public function index() {
        ...
        foreach ($result as $n=> $r) {
            ...
            $result[$n]['str_manage'] = '<a href="' . U("WuliuMenu/add", array("parentid" => $r['id'], "menuid" => I("get.menuid"))) . '">'.L('ADD_SUB_MENU').'</a> | <a href="' . U("WuliuMenu/edit", array("id" => $r['id'], "menuid" => I("get.menuid"))) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete" href="' . U("WuliuMenu/delete", array("id" => $r['id'], "menuid" => I("get.menuid")) ). '">'.L('DELETE').'</a>';
    				...
    				if(count($actions)){
    					foreach ($actions as $mf){
    						if(!(strpos($mf, ".") === 0)){
    							...
    							$class=A($g."/".$m);
    							$adminbaseaction=new \Common\Controller\WuliubaseController();
